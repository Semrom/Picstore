<?php /*** @author Thomas Allam / Romain Semler** @file Galerie.php** @date 25/02/2016** @brief Classe chargé de récupérer une galerie et ses images.*/
	class Galerie {
		...
		public function getGalleryByName($id_user, $nom_galerie) {
			$req_gal = $this->bdd->prepare('SELECT id_galerie FROM galerie WHERE id_user = :id AND nom_galerie = :nomGal');			$req_gal->execute(array('id' => $id_user, 'nomGal' => $nom_galerie));
			$id_gal = $req_gal->fetch();
			$req_gal->closeCursor();
			return $id_gal;
		}		
		public function getImagesFromGallery($id_galerie) {
			$req_gal = $this->bdd->prepare('SELECT * FROM posseder_image WHERE id_galerie = :id');			$req_gal->execute(array('id' => $id_galerie));
			$images = $req_gal->fetchAll();
			$req_gal->closeCursor();
